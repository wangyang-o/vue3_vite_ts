...
<template>
  ...
  <!-- 添加 -->
  <el-drawer
    :title="addOrUpdateFlag === 'add' ? '添加客户' : '修改客户'"
...
import { getClientList, deleteClient, putClient, getInstanceById } from '@/api/client';
...
export default defineComponent({
  ...
  setup: () => {
    ...
    const clientList = async () => {
      const { data }: any = await getClientList(queryParams);
    ...
    const handleEdit = async (clientId: string) => {
      addOrUpdate('edit');
      const { code, data }: any = await getInstanceById(clientId);
      if (code === 0) {
    ...
    const handleDelete = async (clientId: number) => {
      const { code, msg }: any = await deleteClient({ ids: clientId });
      if (code === 0) {
        searchUsername()
        ElNotification({
          title: '提示',
    ...
    const submitDrawer = () => {
      (drawerFormRef.value as any).validate(async (valid: boolean) => {
        ...
        if (addOrUpdateFlag.value === 'add') {
          const { code, msg }: any = await putClient(drawerForm.value);
          if (code === 0) {
            handleDrawerClose();
            searchUsername();
            ElNotification({
