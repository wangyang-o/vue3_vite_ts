...
<template>
        ...
        <el-form @submit.native.prevent :inline="true">
          <el-row type="flex">
            <el-col :span="5">
              <el-input
                clearable
                @keyup.enter="searchpriceName"
                @clear="searchpriceName"
                size="small"
                v-model="priceName"
                placeholder="价格名称"
              ></el-input>
  ...
  <el-card class="m-1">
    <el-table size="small" :data="tableData" style="width: 100%" empty-text="没有数据了...">
      ...
      <el-table-column align="center" prop="id" width="250px" label="价格ID"></el-table-column>
      <el-table-column align="center" prop="priceName" label="价格名称"></el-table-column>
      <el-table-column align="center" prop="priceCode" label="价格（元）"></el-table-column>
      ...
      <el-table-column align="center" label="操作">
        <template #default="scope">
          ...
          <el-popconfirm
            ...
            icon="el-icon-info"
            iconColor="red"
            :title="'确定删除 ' + scope.row.priceName + ' 的所有信息吗？'"
            trigger="click"
  ...
  <!-- 添加 -->
  <el-drawer
    :title="addOrUpdateFlag === 'add' ? '添加价格' : '修改价格'"
    ...
    <div class="mx-5">
      ...
      >
        <el-form-item label="价格名称:" prop="priceName">
          <el-input v-model="drawerForm.priceName" autocomplete="off" placeholder="请输入姓名"></el-input>
        </el-form-item>
        <el-form-item label="价格（单价）:" prop="priceCode">
          <el-input v-model="drawerForm.priceCode" autocomplete="off" placeholder="请输入价格"></el-input>
        </el-form-item>
      </el-form>
      <el-row :gutter="10">
        <el-col :span="12">
...
import { getPriceList, deletePrice, putPrice, getInstanceById } from '@/api/price';
...
export default defineComponent({
  ...
  setup: () => {
    ...
    const priceList = async () => {
      const { data }: any = await getPriceList(queryParams);
    ...
    const handleEdit = async (priceId: string) => {
      addOrUpdate('edit');
      const { code, data }: any = await getInstanceById(priceId);
      if (code === 0) {
    ...
    const handleDelete = async (priceId: number) => {
      const { code, msg }: any = await deletePrice({ ids: priceId });
      if (code === 0) {
        searchpriceName()
        ElNotification({
          title: '提示',
    ...
    // Drawer表单数据
    const drawerForm = ref({ priceName: '', priceCode: undefined });
    // 验证规则
    const rules = {
      priceName: [
      ...
      priceCode: { required: true, message: "请输入价格（数字）", trigger: "blur" },
    ...
    const submitDrawer = () => {
      (drawerFormRef.value as any).validate(async (valid: boolean) => {
        ...
        if (addOrUpdateFlag.value === 'add') {
          const { code, msg }: any = await putPrice(drawerForm.value);
          if (code === 0) {
            handleDrawerClose();
            searchpriceName();
            ElNotification({
